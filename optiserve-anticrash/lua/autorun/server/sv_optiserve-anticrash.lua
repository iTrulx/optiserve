...
function HPP.DeLag()
    if aprint then
	   aprint(Color(255, 0, 0), "[OptiServe] | ", {"root", "superadmin"}, "De-lagging server.")
    end
...
function HPP.StopLag()
    if aprint then
        aprint(Color(255, 0, 0), "[OptiServe] | ", nil, "The server was lagging so all entities have been frozen.")
        aprint(Color(255, 0, 0), "[OptiServe] | ", {"root", "superadmin", "headadmin", "admin", "mod"}, "If this keeps occuring someone may be trying to crash the server.")
    end
	for k, v in pairs(ents.GetAll()) do
